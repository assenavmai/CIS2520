...
/*preconditions: a file name and path is passed in as the parameters
 *postconditions: new node is stored into the struct
*/
AVL * createNode(char * filename, char * filepath)
{
    /*Initialization of vairables*/
    AVL * newInfo;

    /*if memory cannot be malloced, exit the program*/
    if((newInfo = malloc(sizeof(AVL))) == NULL)
    {
...
int compare(void * data1, void * data2)
{
    ...
    TreeNode * a = (TreeNode *)data1;
    TreeNode * b = (TreeNode *)data2;
    ...
    /*if the filenames are the same, return 0*/
    if(strcmp(a->filepath, b->filepath) == 0)
    {
        /*Compare by filepath*/
...
AVL * insert(AVL * tree, char * filename, char * filepath)
{
    if(tree == NULL)
    {
        tree = createNode(filename, filepath);
    }
}